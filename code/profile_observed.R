...
message("\n=== PDF Page 4 Structure ===")
message("Total text elements: ", nrow(raw_data_original))
message("Y coordinate range: ", min(raw_data_original$y), " to ", max(raw_data_original$y))
message("X coordinate range: ", min(raw_data_original$x), " to ", max(raw_data_original$x))
...
message("After cleaning: ", nrow(raw_data), " text elements")

# Show sample of data around expected table area
message("\nSample of text in table region (y: 190-250):")
...
message("\nSearching for period headers (pattern: ##A##B or ##B##A):")
period_headers <- raw_data %>%
  filter(grepl("^[0-9]{2}[AB][0-9]{2}[AB]$", text)) %>%
  arrange(y, x) %>%
  select(x, y, text)
message("Found ", nrow(period_headers), " period headers:")
...
process_table_observed <- function(df, column_names) {
  ...
  message("DEBUG process_table_observed: Input has ", nrow(df), " rows")
  ...
  message("DEBUG: Column names assigned: ", paste(column_names, collapse = ", "))
  message("DEBUG: Sample rows before filtering:")
  print(head(df, 20))
  ...
  message("DEBUG: After filtering, ", nrow(df_clean), " rows remain")
...
message("DEBUG: Extracted column headers: ", paste(top_cols, collapse = " | "))
...
message("DEBUG: Top table raw extraction - ", nrow(df_top_raw), " rows")
message("DEBUG: Top table columns: ", paste(names(df_top_raw), collapse = ", "))

df_top_processed <- process_table_observed(df_top_raw, top_cols)

message("DEBUG: Top table processed - ", nrow(df_top_processed), " rows")
message("DEBUG: Expected 15 rows (5 indicators × 3 measure types)")
...
message("DEBUG: First few rows of processed top table:")
print(head(df_top_processed, 10))
...
message("\n=== Extracting Bottom Table (Maltreatment indicators) ===")
...
message("DEBUG: Bottom table raw extraction - ", nrow(df_bottom_raw), " rows")

# Process bottom table
df_bottom_processed <- process_table_observed(df_bottom_raw, bottom_cols)

message("DEBUG: Bottom table processed - ", nrow(df_bottom_processed), " rows")
message("DEBUG: Expected 6 rows (2 indicators × 3 measure types)")
...
message("\n=== Observed Performance CSV processing complete ===")
...
message("\n=== Observed Performance Processing Complete ===")
