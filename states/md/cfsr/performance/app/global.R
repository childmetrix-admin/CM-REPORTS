...
# Get state and profile from URL parameters (passed from parent page)
# Default to MD and latest if not provided
...
if (!is.null(rds_path)) {
  ...
  if (length(period_dirs) == 0) {
    stop("No data found. Please run cfsr-profile.R first, then prepare_app_data.R")
  }
