...
# Save for Shiny app - Save to both locations
# 1. Development location (r_cfsr_profile)
output_dir_dev <- "D:/repo_childmetrix/r_cfsr_profile/shiny_app/data"
if (!dir.exists(output_dir_dev)) {
  dir.create(output_dir_dev, recursive = TRUE)
}
output_file_dev <- file.path(output_dir_dev, "cfsr_indicators_latest.rds")
saveRDS(app_data, output_file_dev)
message("Saved prepared data to DEV: ", output_file_dev)

# 2. Production location (r_cm_reports)
output_dir_prod <- "D:/repo_childmetrix/r_cm_reports/md/cfsr/performance/app/data"
if (!dir.exists(output_dir_prod)) {
  dir.create(output_dir_prod, recursive = TRUE)
}
output_file_prod <- file.path(output_dir_prod, "cfsr_indicators_latest.rds")
saveRDS(app_data, output_file_prod)
message("Saved prepared data to PROD: ", output_file_prod)
